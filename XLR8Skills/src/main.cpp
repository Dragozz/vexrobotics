...
void moveArmsPotent(double speed, double angle) {
  while(potentArm.angle() < angle) {
    armMotor.spin(reverse, speed, percentUnits::pct);
  }
...
void flipOutTray() {
  ...
  intakeMotor1.spin(reverse, intakeSpeed, vex::percentUnits::pct);
  intakeMotor2.spin(reverse, intakeSpeed, vex::percentUnits::pct);
  moveArms(armSpeed, -1.3, true);
...
void intakeOneCube() {
  while(true) {
    ...
    if(sensorValue < 2800 && sensorValue > 1) {
      ...
      intakeMotor1.spin(forward, intakeSpeed, vex::velocityUnits::pct);
      intakeMotor2.spin(forward, intakeSpeed, vex::velocityUnits::pct);
      chassis.drive(forward, 35, velocityUnits::pct);
    }
  }
}
...
void usercontrol(void) {
  ...
  while (1) {
    ...
    if(Controller.ButtonLeft.pressing()) { //small & medium tower
      ...
      while(true) {
        if(armMotor.position(rev) > -0.95) {
          while(armMotor.position(rev) > -0.95) {
            armMotor.spin(reverse, armSpeed, velocityUnits::pct);
          ...
          while(armMotor.position(rev) <= -0.95) {
            armMotor.spin(forward, armSpeed, velocityUnits::pct);
            if(Controller.ButtonRight.pressing())
              break;
          }
          break;
      ...
      armMotor.stop();
      //armMotor.rotateTo(-0.95, rev, armSpeed, velocityUnits::pct, true);
    }
    ...
    if(Controller.ButtonUp.pressing()) { //tall tower
      //armMotor.setTimeout(2000, msec);
      while(armMotor.position(rev) > -1.25) {
        if(Controller.ButtonRight.pressing())
          break;
        armMotor.spin(reverse, armSpeed, velocityUnits::pct);
      }
      armMotor.stop();
      //armMotor.rotateTo(-1.25, rev, armSpeed, velocityUnits::pct, true);
    }
    //move arm back down
